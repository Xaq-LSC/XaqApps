                ...
                try:
                    ...
                    for attr in root.findall(".//{http://es.com/digistar/2012/03}DsAttributeValue"):
                        ...
                        if obj is not None and val is not None and val.text == "true":
                            name = obj.text.strip()
                            interpreted_panel.insert(tk.END, f"🎯 Button '{name}' is ON\n")
                            interpreted_panel.see(tk.END)
                            if name == "Taygete": load_playlist("Taylor Swift")
                            elif name == "asteroidBacchus": load_playlist("Bad Bunny")
                            elif name == "Chaldene": load_playlist("Chappell Roan")
                            elif name == "Hegemone": start_playlist()
                            elif name == "Telesto": stop_playlist()
